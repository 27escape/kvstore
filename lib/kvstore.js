...
class KVStore {
  constructor ({ filename, namespace, indent = 2 }) {
    ...
    try {
      ...
      this.store.set('store.accesses', about.accesses + 1).save(this.indent);
      this._unlockFile();
    } catch (err) {
      this._unlockFile();
      // error(err);
  ...
  /**
   * { check that the key does not contain a number with period/dot separators }
   *
   * @method     _checkKey
  ...
  _checkKey (key) {
    if (key) {
      switch (typeof key) {
        case 'number' :
          throw new Error('Cannot have a number as a key');
          break; // eslint-disable-line
        case 'string' :
          if (key.match(/(^\d+$|\b\d+\.|\.\d+\b)/)) {
            throw new Error('Cannot use numbers and "." in the key');
          }
          break;
        default:
          throw new Error('Only strings allowed as a key');
      }
    }
  }
