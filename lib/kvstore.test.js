...
describe('KV put + get', () => {
  ...
  it( 'namespace cannot start with period', () => {
    expect(() => expect(new KVStore({ filename: _makeTestFile(testnum), namespace: '.Jest' + testnum })).toThrow(Error))
  });

  it( 'namespace cannot contain string.number type construct eg namespace.1', () => {
    expect(() => expect(new KVStore({ filename: _makeTestFile(testnum), namespace: `.Jest.{testnum}` })).toThrow(Error))
  });
...
describe('KV export', () => {
  ...
  it('provides JSON string matching contents', () => {
    expect(exportdata).toBe('{"a":{"b":{"c":{"d":1}}}}');
  });
});
